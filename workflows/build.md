...
- **Cowork(AI)サンドボックスではビルド不可**（nuget.org/MSダウンロードが
  プロキシ遮断、.NET SDK導入も不可）。AIセッションでは静的検証まで行い、
  実ビルド/スモークは Windows 側で `py -3 .\tools\smoke_test.py` を実行
  してもらう。
- **Coworkマウントの罠**: AIのファイルツールで既存ファイルを編集すると、
  サンドボックス側マウントが**旧サイズで切り詰めた内容**を返すことがある
  （stat キャッシュ stale）。この状態で `git add` すると壊れたファイルを
  コミットする。対策= `wc -c`/`tail` で完全性確認後、`git show HEAD:` から
  決定論的に再生成した `.new` を `mv` で置き換えてから git 操作。新規作成
  ファイルは影響なし。
