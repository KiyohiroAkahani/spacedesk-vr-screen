...
- **検証の一発化**: `py -3 .\tools\smoke_test.py` を追加（stop→clean_build→
  6秒タイムボックス起動→kill→判定）。PASS条件 = `[INFO] LensDistortion=`
  （デバイス＋全シェーダーコンパイル成功後にのみ出る行）と `[DIAG]` があり
  `[FATAL]` が無いこと。シェーダーは実行時コンパイルなので、ビルド緑だけ
  では検証にならない（この行の有無で判定する）。
- **キャリブレーション機能（2026-06-10, 課題A/B対応）**: `T`=テスト
  パターン（十字=融像ターゲット, 同心円/グリッド=曲率, 橙枠=コンテンツ端）、
  `I/K/J/L`=両眼共通の上下左右ナッジ（0.5%/押, スマホの置きズレ補正）、
  `U/O`=右眼像の上/下（0.25%/押, 垂直視差はわずか0.3%で融像破綻する
  最有力犯）、`B/N`=曲率K1/K2の強/弱（×1.10, 比率保持）、IPD矢印は
  1%→**0.5%/押**に微細化。全調整値は config.json に**自動保存**される
  （手書き編集不要に）。すべて Ctrl+Alt+Shift+ 前提。
- **数理（docs/IPD_DISTORTION_ANALYSIS.md）**: 「IPDシフトで曲面が平面化」
  はK1/K2不変でも実際に起きる：旧状態は視線が歪み軸の8%外側にあり視線上の
  縦線が湾曲して見えていた（0.16mm）。中心整列後は構造的にゼロ。対策=Bキー
  でK1を上げ周辺曲率で補う（まず0.26〜0.33帯）。入れ過ぎ注意：シフト過多は
